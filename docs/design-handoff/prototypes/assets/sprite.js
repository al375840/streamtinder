/* ============================================================
   STREAMER TINDER — sprite engine
   Native 24x30 pixel sprite. Render to canvas, scale via CSS.
   Deterministic nick -> (body, skin, hair, pose) hash.
   ============================================================ */
...
(function () {
  ...
  const SKIN_COLORS = ['#ffd5b0', '#e8a878', '#a06a3a', '#5a3a1f'];
  const HAIR_COLORS = ['#2a1810', '#6b3a1a', '#d4a23d', '#b8341f', '#4a4a6a', '#ff3c8b'];
  const PANTS_COLORS = ['#3a2a5a', '#1f3a5a', '#5a3a1f', '#2a2a2a'];
  const POSES = ['idle', 'arms_up', 'wave', 'cheer', 'point', 'hands_hips'];
  ...
  function makePoseBuffer(spec, pose) {
    const W = 24, H = 30;
  ...
  function paintBuffer(ctx, ox, oy, buf, W, H) {
    for (let y = 0; y < H; y++) {
      for (let x = 0; x < W; x++) {
        const c = buf[y * W + x];
  ...
  // ---- High-level: render sprite onto a target canvas ------
  // canvas: existing HTMLCanvasElement (sized in CSS via scale)
  // The canvas backing buffer should be 24*scale x 30*scale, but
  // we draw at NATIVE 24x30 and let CSS scale via image-rendering: pixelated.
  function renderToCanvas(canvas, nick, opts) {
    opts = opts || {};
    const spec = typeof nick === 'string' ? spriteFor(nick, opts.packPalette) : nick;
  ...
  // ---- Helper: build an <canvas class="sprite"> element and render
  function makeSpriteEl(nick, opts) {
    opts = opts || {};
    ...
    const spec = renderToCanvas(c, nick, opts);
    c.dataset.nick = typeof nick === 'string' ? nick : '';
    c.dataset.pose = opts.pose || spec.pose;
    return c;
  }

  // ---- Expose ---------------------------------------------
  window.SpriteEngine = {
    hashNick, spriteFor, renderToCanvas, makeSpriteEl,
    BODY_COLORS, SKIN_COLORS, HAIR_COLORS, POSES,
    darken, lighten
  };
})();
